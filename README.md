...
- **版本更新说明：** [CHANGELOG.md](CHANGELOG.md)
...
**大模型设置随便填模型名可以吗？**  
不可以。需要 OpenAI 兼容接口，且模型须支持识图（如 gpt-4o）。纯文本模型通常无法描述图片。详见 [CHANGELOG](CHANGELOG.md) / 软件内「格式说明」。
...
详细条目（每个版本「更新了什么 / 解决了什么」）见 **[CHANGELOG.md](CHANGELOG.md)**。

| 版本 | 一句话摘要 |
|------|------------|
| **v1.7.11**（当前） | 表格短分隔/等值行误折修复；图片 alt 含 `]` 修复 |
| v1.7.10 | 设置说明：大模型须识图；Azure/高级项释义与 FAQ |
| v1.7.9 | 上市加固：仓库一致、ZIP 路径穿越跳过、OCR 打包硬校验 |
| v1.7.8 | ZIP 中文文件名；Excel 成对合并列折叠 |
| v1.7.7 | 表宽/对齐；Notebook markdown；设置中文与格式说明分页签 |
| v1.7.6 | 中间列 `\|` 合并；货币/科学计数；CLI 扫描 PDF 后检 |
| v1.7.5 | 代码块内表格免误伤；转义幂等；货币符号 |
| v1.7.4 | 空表头/单元格竖线；Excel 日期百分比货币 |
| v1.7.3 | 公式回退；CLI 预检；keep_data_uris；插件冻结列表 |
| v1.7.2 | 保留内嵌图；插件列表；正式文档与发布 |
| v1.7.0 | 高级设置、CLI、Docker |
| v1.6.x | Azure / 大模型 GUI、Office 增强 |
